...
class FeatureExtractorCore:
    ...
    def get_dx(sef, row):
        ...
        if (change_code in ['-1', 'NA']):
            ...
            dd = {'1': 'NL', '2': 'MCI', '3': 'AD', '4': 'MCI', '5': 'AD', '6': 'AD', '7': 'NL', '8': 'MCI', '9': 'NL',
                  '-1': 'NA', 'NA': 'NA'}
            newdx = dd[str(int(float(change_code)))] ## IMP
    ...
    def patientwise(self, derive_dx=True, include_col_list=None, genTest=False):
        ...
        cols = ['Ventricles', 'Hippocampus', 'WholeBrain', 'Entorhinal', 'Fusiform', 'MidTemp', 'ICV', 'Age_r',
                'CDRSB','ADAS11','ADAS13','MMSE','RAVLT_immediate','RAVLT_learning','RAVLT_forgetting','RAVLT_perc_forgetting',
                'FAQ','MOCA','AV45','FDG','ABETA_UPENNBIOMK9_04_19_17','TAU_UPENNBIOMK9_04_19_17','PTAU_UPENNBIOMK9_04_19_17',
                'DX']
        ...
        for pt in groups.keys():
            ...
            # patient_mri.to_csv('data/patient_mri_THISISIT.csv', index=False) ## IMP
            if (genTest):
                features, isvalid = self.prepare_data_rf_mtp_test(patient_mri)
                labels = isvalid
            else:
                features, labels = self.prepare_data_rf_mtp(patient_mri)
            # with open('data/patient_mri_THISISIT.txt', 'a') as f: ## IMP
    ...
    def prepare_data_rf_mtp(self, pat_df):
        ...
        def temp(arow0, arow1):
            ...
            arow0_temp = arow0.drop(['DX','Age_r'])
            features, suc = string2float(list(arow0_temp))
            age1 = arow1['Age_r']
        ...
        for i in range(0, len(pat_df)):
            # arowi = pat_df.iloc[i]
            # features, suc = string2float(list(arowi))
            # DX1 = int(arowi['DX'])
            # labels =  DX1
